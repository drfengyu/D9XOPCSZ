    ...
    public partial class MainFrm : Form
    {
        ...
        private void MainFrm_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Global.client1 = new AsyncTcpClient(IPAddress.Parse("192.192.192.2"), 8080);
                ...
                Global.client2 = new AsyncTcpClient(IPAddress.Parse("192.192.192.1"), 8080);
                ...
                Global.client3 = new AsyncTcpClient(IPAddress.Parse("192.192.192.4"), 8080);
        ...
        private void CheckConnected(object ob)
        {
            while (true)
            {
                    try
                    {
                        if (Global.client1.Connected)
                        {
                            if (Global.client2.Connected)
                            {
                                if (Global.client3.Connected)
                                {
                                    ...
                                    ShowStatus("3#焊机已断开,请重启焊机软件", Color.Red, 4);
                                        Global.client4 = new AsyncTcpClient(IPAddress.Parse("192.192.192.3"), 8080);
                                        Global.client4.PlaintextReceived -= client4_PlaintextReceived1;
                                        Global.client4.ServerConnected -= client4_ServerConnected;
                                        Global.client4.ServerDisconnected -= client4_ServerDisconnected;
                        ...
                        {
                        Global.plc2.Write("MW65199", Convert.ToInt16(1));
                        ShowStatus("2#焊机已断开,请重启焊机软件", Color.Red, 4);
                            Global.client1 = new AsyncTcpClient(IPAddress.Parse("192.192.192.2"), 8080);
                            Global.client1.PlaintextReceived -= Client1_PlaintextReceived1;
                            Global.client1.ServerConnected -= Client1_ServerConnected;
                            Global.client1.ServerDisconnected -= Client1_ServerDisconnected;
                    ...
                    if (ison)
                    {
                      bool A=  Global.plc1.Write("MW2314", Convert.ToInt16(1)).IsSuccess;
                        ...
                        if (!A)
                        {
                            ...
                            Log.WriteLog_ServerInfo("1#PLC断线重连.");
                        }
                        if (!B)
                        {
                            Global.plc2.ConnectServer();
                            Log.WriteLog_ServerInfo("2#PLC断线重连.");
                        }
                        if (!C)
                        {
                            Global.plc3.ConnectServer();
                            Log.WriteLog_ServerInfo("3#PLC断线重连.");
                        }
                        ison = false;
                    }
                    else
        ...
        private ResultModel<PassModel> loading_check(string SN, string left_rail, string right_rail, string fixture)
        {
            lock (lock_check)
            {
                try
                {
                    ...
                    var a = loadThree(new LoadThreeModel()
                    {
                        barCode = SN,
        ...
        private ResultModel<PassModel> loadThree(LoadThreeModel LoadThree)
        {
            ...
            if (recvObj["rc"].ToString() == "000")
            {
                ...
                else
                {
                    ...
                    Log.WriteLog("上料确认异常信息：" + recvObj["rm"].ToString());
                    _homefrm.UpDatelabelcolor(Color.Red, "上料确认NG!", "lb_上料确认");
                    Oh.OEENG(LoadThree.barCode, LoadThree.left_rail, LoadThree.right_rail, recvObj["rm"].ToString(), "上料确认");
                    ...
                    if (Trg_check == 1)
                    {
                       ...
                       string right_rail = ToASCII(right_sn.Content).Trim();
                        ...
                        if (Global.client1.Connected)
                        {
                            ...
                            Log.WriteLog_ServerInfo("Barcode:"+barcode+"2#焊后触发焊机，请求数据。");
                        }
                        ...
                        //if (PassModelLists.ContainsKey(barcode))
                        //{
                        //    left_rail = PassModelLists[barcode].LeftRail;
                        //    right_rail = PassModelLists[barcode].RightRail;
                        //}
                        //else
                        //{
                        //    OperateResult<short[]> left_sn = Global.plc2.ReadInt16("MW65400", 20);
                        ...
                        //    Log.WriteLog_ServerInfo("1#Barcode:" + barcode + "请求数据无,读取plc。"+left_rail+","+right_rail);
                        //}
                    ...
                    if (Trg_check == 1)
                    {
                       ...
                       string right_rail = ToASCII(right_sn.Content).Trim();
                        ...
                        if (Global.client3.Connected)
                        {
                            ...
                            Log.WriteLog_ServerInfo("Barcode:" + barcode + "焊后触发4#焊机，请求数据。");
                        }
                           ...
                           
                        {
                            ...
                            if (pass_station(barcode, left_rail, right_rail, fixture, cavity, ResultModel.Model))
                            {
                                ...
                                string inStr2 = string.Format("insert into Hans_Info values('{0}','{1}','{2}','3')",
                                   DateTime.Now.ToString("yyyy-MM-dd HH:mm"), barcode, fixture);
                                SQL.ExecuteUpdate(inStr2);
                                Log.WriteCSV_HansInfo(barcode + "," + fixture + "," + "4" + ",0" + cavity);
                                #region SFC
                                Sc.UpdateUI("参数上抛产品码：" + barcode, "Rtxt_Send_Pass");
                                //PassModelLists[barcode].LeftRail PassModelLists[barcode].RightRail
                                Sc.UpdateUI("参数上抛左键码：" + left_rail, "Rtxt_Send_Pass");
                       ...
                       string right_rail = ToASCII(right_sn.Content).Trim();
                        ...
                        if (ResultModel.Result)
                        {
                            ...
                            if (pass_station(barcode, left_rail,right_rail, fixture, cavity, ResultModel.Model))
                            {
                                ...
                                Global.b_Trace = true;
                                string inStr2 = string.Format("insert into Hans_Info values('{0}','{1}','{2}','4')",
                                   DateTime.Now.ToString("yyyy-MM-dd HH:mm"), barcode, fixture);
                                SQL.ExecuteUpdate(inStr2);
        ...
        private ResultModel<FlHelper.Models.ParaInfo[]> pass_Trace(string SN, string left_rail, string right_rail, string fixture, string head_id)
        {
            lock (lock_Trace)
            {
                ...
                try
                {
                    ...
                    //int Count = 150 + heightArray.Length;
                    ...
                    while (dt.Rows.Count < 15)
                    {
                        ...
                        { 
                            ...
                            sql = "SELECT TOP 15 * FROM [ZHH].[dbo].[HGData] ORDER BY DateTime DESC";
                    ...
                    for (int i = 0; i < 150; i++)
                    {
                        ...
                        //trace_ua.data.insight.results[i].test = dt.Rows[i][2].ToString();
                        switch (i / 10)
                        {
                    ...
                    if (HgDataHeights.ContainsKey(SN))
                    {
                        ...
                        for (int i = 0; i < heightArray.Length; i++)
                        {
                            ...
                            trace_ua.data.insight.results[i + 150].test = heightArray[i].test;
                            trace_ua.data.insight.results[i + 150].sub_test = heightArray[i].sub_test;
                            trace_ua.data.insight.results[i + 150].units = heightArray[i].units;
                            trace_ua.data.insight.results[i + 150].value = heightArray[i].value;
                        }
                    }
                    else
                    {
                    ...
                    else
                    {
                        ...
                        Oh.OEENG(SN, left_rail, right_rail, head_id + "#焊机Trace上传失败", "Trace上传");
                ...
                catch (Exception ex)
                {
                    ...
                    Oh.OEENG(SN,left_rail,right_rail,ex.Message,"Trace上传");
        ...
        private bool pass_station(string SN, string left_rail, string right_rail, string fixture,string cavity,FlHelper.Models.ParaInfo[] parainfos=null)
        {
            lock (lock_pass)
            {
                try
                {
                    ...
                    if (errMsg.Contains("请求超时")&& string.IsNullOrEmpty(callResult))
                    {
                        RequestAPI3.CallBobcat2(Global.passURL, send_Pass, Global.hashCode, out callResult, out errMsg);
                        Log.WriteLog("A工位 API to Equipment 過站報告:" + callResult + ",barcode：" + SN + "," + errMsg);
                    }
                    ...
                    else
                    {
                        ...
                        Oh.OEENG(SN, left_rail, right_rail, recvObj["rm"].ToString(), "过站");
                        return false;
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    Oh.OEENG(SN, left_rail, right_rail, ex.Message, "过站");
                    return false;
                }
            }
